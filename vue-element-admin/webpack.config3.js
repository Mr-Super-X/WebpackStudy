...
const { WebpackManifestPlugin } = require("webpack-manifest-plugin"); // chunk映射关系插件
...
const config = {
  ...
  plugins: [
    ...
    new VueLoaderPlugin(),
    // style-loader会将样式内嵌到文件中，使用该插件可以对style进行分离，提取到单独的css文件中
    ...
    // 利用HardSourceWebpackPlugin在node_modules/.cache目录中为模块提供中间缓存
    // 为了查看结果，您需要使用此插件运行 webpack 两次：第一次构建将花费正常的时间。第二个构建将显着更快。
